...
$(document).ready(function () {
    ...
    layui.use(['element', 'flow', 'layer', 'form'], function () {
...

        {
                            ...
                            success: function (result) {
                                ...
                                layui.each(result.reverts, function (i, revert) {
                                    ...
                                    for (let i = 0; i < len; i++) {
                                        ...
                                        str1 = str1 + '<div class="at_content" style="width: 96%">\n' +
                                    ...
                                    for (let i = 0; i < revert.questionZanCaiList.length; i++) {
                                        if (revert.questionZanCaiList[i].userId === $("#userId").text() + '') {
                                            zanCaiState = revert.questionZanCaiList[i].caiZan;
                                            if (zanCaiState === 'zan')
                                                zanStr = '<i class="iconfont icon-dianzan ART_ZCbtn" data_name="zan" style="font-size: 20px;color: rgb(102,71,238);"></i>';
            ...
            //问题评论选项卡 点击事件
            $("#look_revert").click(function () {
                replyEditorArr.length = 0;
                cmtFlowLoad(basePath + "/revert/findRevert");
            });

            //回复富文本编辑器 回复按钮点击事件
            /*  $("#contentBox").on('click','.ART_replyBtn',function (ev) {
                  let index = parseInt($(this).prev().text());
                  let rid = parseInt($(this).next().text());
                  ...
                  let data;
                  if (type === 'son'){
                      data = {'questionId':questionId, 'userId':userId, 'revertContent':contentHtml, 'rid':rid, 'replyPerson':replyPerson};
                  }else {
                      data = {'questionId':questionId, 'userId':userId, 'revertContent':contentHtml, 'rid':rid};
                  }
              ...
              //回复工具栏 收起/展开 按钮点击事件
              $("#contentBox").on('click','.replyFlex',function () {
  ...
  */
            //评论工具栏 评论按钮点击事件
            $("#contentBox").on('click', '.replyBtn', function () {
            ...
            //评论工具栏 展开/收起 按钮点击事件
            $("#contentBox").on('click', '.ART_flexBtn', function () {
                let state = $(this).prev().text();
                let type = $(this).attr("data_type");
                if ('false' === state) {
                    $(this).prev().text('true');
            ...
            $("#contentBox").on("mouseover", ".at_content", function () {
            ...
            $("#contentBox").on("mouseout", ".at_content", function () {
